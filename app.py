...
# =============================================================================
# ---------------------------------------------------------------------------
# ملاحظة توثيقية (scoring.py سابقًا):
...
# =============================================================================
# ---------------------------------------------------------------------------
# ملاحظة توثيقية (analysis.py سابقًا):
...
# =============================================================================
# ---------------------------------------------------------------------------
# ملاحظة توثيقية (decision_engine.py سابقًا):
...
# =============================================================================
# ---------------------------------------------------------------------------
# ملاحظة توثيقية (response_generator.py سابقًا):
...
# =============================================================================
# ---------------------------------------------------------------------------
# ملاحظة توثيقية (support_widgets.py سابقًا):
...
def render_report_panel(colors: dict):
    ...
    with st.container(key="report_panel"):
        ...
        with c1:
            st.markdown("### 📢 البلاغات والاستفسارات")
        with c2:
...
def inject_css():
    st.markdown(
        ...
        .bayan-approved-topline {{
            display: flex;
            justify-content: space-between;
            align-items: center;
            margin-bottom: 0.6rem;
        }}
        .bayan-approved-badge {{
            display: inline-flex;
            align-items: center;
            gap: 0.3rem;
...
REVIEWER_NAME = "م. عبير مقبول الصقري"
...
# =============================================================================
# قسم "حالات معتمدة رسميًا" — يظهر في الصفحة الرئيسية
# =============================================================================
HOME_SHOWCASE_CLAIM_IDS = ["CLM008", "CLM006", "CLM010"]


def get_home_showcase_cases(limit=3):
    """
    يجهّز 2-3 حالات من نفس بيانات المنصة لعرضها في الصفحة الرئيسية كنماذج
    "معتمدة رسميًا" بواسطة المختص المسؤول. إن كانت الحالة معتمدة فعليًا
    خلال الجلسة الحالية (عبر صفحة «الردود الرسمية» أو «العرض التجريبي»)
    تُستخدم بيانات الاعتماد الفعلية، وإلا يُعتمد على آخر حدث مسجّل للحالة
    مع اعتبار {REVIEWER_NAME} هو المختص المعتمِد الافتراضي في هذا العرض.
    """
    df = get_claims_df()
    cases = []
    for cid in HOME_SHOWCASE_CLAIM_IDS[:limit]:
        matches = df[df["id"] == cid]
        if matches.empty:
            continue
        row = matches.iloc[0].to_dict()

        approval = st.session_state.approvals.get(cid)
        if approval and approval.get("reviewer"):
            reviewer = approval["reviewer"]
            approval_time = approval["approval_time"]
        else:
            ev = events_df[events_df["claim_id"] == cid].sort_values("timestamp")
            approval_time = str(ev.iloc[-1]["timestamp"]) if not ev.empty else str(row["timestamp"])
            reviewer = REVIEWER_NAME

        draft = st.session_state.drafts.get(cid)
        if not draft:
            evidence_list = sources_df[sources_df["related_claim"] == cid].to_dict(orient="records")
            draft = generate_response(row, row["agency_name"], evidence_list)

        cases.append({"row": row, "reviewer": reviewer, "approval_time": approval_time, "draft": draft})
    return cases


def render_approved_showcase_card(case):
    row = case["row"]
    draft = case["draft"]
    reviewer = case["reviewer"]
    approval_time = case["approval_time"]
    initial = (reviewer or "؟").replace("م.", "").strip()[:1] or "؟"

    try:
        time_label = pd.to_datetime(approval_time).strftime("%Y-%m-%d %H:%M")
    except Exception:
        time_label = str(approval_time)

    claim_text = row["content"]
    claim_preview = claim_text[:130] + ("..." if len(claim_text) > 130 else "")
    statement_preview = draft["statement"][:220] + ("..." if len(draft["statement"]) > 220 else "")

    st.markdown(
        f"""
        <div class="bayan-approved-card">
            <div class="bayan-approved-topline">
                <span class="bayan-approved-badge">✓ معتمد رسميًا</span>
                {risk_pill(row['risk_score'])}
            </div>
            <div class="bayan-approved-claim">{claim_preview}</div>
            <div class="bayan-approved-statement">{statement_preview}</div>
            <div style="font-size:0.78rem; color:{COLORS['muted']}; margin-bottom:0.7rem;">
                الجهة: {row['agency_name']} &nbsp;|&nbsp; الخدمة: {row['service']}
            </div>
            <div class="bayan-approved-meta">
                <div class="bayan-approver-chip">
                    <div class="bayan-approver-avatar">{initial}</div>
                    <div>
                        <div class="bayan-approver-name">{reviewer}</div>
                        <div class="bayan-approver-role">{REVIEWER_ROLE}</div>
                    </div>
                </div>
                <div class="bayan-approved-time">اعتماد بتاريخ {time_label}</div>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def render_home_approved_showcase():
    cases = get_home_showcase_cases()
    if not cases:
        return
    st.markdown("#### حالات معتمدة رسميًا")
    st.caption("نماذج من بيانات العرض التجريبي بعد اكتمال دورة الاستجابة الكاملة واعتمادها بشريًا.")
    cols = st.columns(len(cases))
    for col, case in zip(cols, cases):
        with col:
            render_approved_showcase_card(case)
