...
ملف واحد يحتوي على كل منطق المنصة: محرك الدرجات، التحليل التنبؤي،
محرك القرار، مولّد المسودات، لوحة البلاغات والاستفسارات، المساعد
الآلي بَيَان، وواجهة Streamlit الكاملة.

هذا نموذج أولي تجريبي (Prototype) لأغراض العرض في هاكاثون حكومي.
...
# =============================================================================
# محرك حساب الدرجات (scoring.py سابقًا)
# =============================================================================
# -*- coding: utf-8 -*-
"""
scoring.py
...
# =============================================================================
# التحليل التنبؤي وتحليلات زمن الاستجابة (analysis.py سابقًا)
# =============================================================================
# -*- coding: utf-8 -*-
"""
analysis.py
...
# =============================================================================
# محرك القرار الذكي (decision_engine.py سابقًا)
# =============================================================================
# -*- coding: utf-8 -*-
"""
decision_engine.py
...
# =============================================================================
# مولّد المسودة الرسمية (response_generator.py سابقًا)
# =============================================================================
# -*- coding: utf-8 -*-
"""
...
# =============================================================================
# لوحة البلاغات والاستفسارات + المساعد الآلي بَيَان (support_widgets.py سابقًا)
# =============================================================================
# -*- coding: utf-8 -*-
"""
support_widgets.py
===================
مكوّنات مشتركة تظهر في جميع صفحات منصة بَيَان | BAYAN:

1. لوحة "البلاغات والاستفسارات" - عائمة على الجهة اليمنى من الشاشة.
2. المساعد الآلي "بَيَان" - عائم أسفل الجهة اليسرى من الشاشة، برد آلي
   مبني على قائمة أسئلة وأجوبة ثابتة (Rule-Based FAQ)، دون أي اتصال
   بنموذج لغوي خارجي.

كلا المكوّنين مصمم ليتّبع نفس الهوية البصرية للمنصة (COLORS) ويُستدعى
مرة واحدة من app.py بعد عرض كل صفحة حتى يبقى ثابتًا في كل الصفحات.
"""



# =============================================================================
# قاعدة أسئلة وأجوبة ثابتة للمساعد الآلي "بَيَان"
# كل عنصر: (كلمات مفتاحية للمطابقة، نص الإجابة)
# =============================================================================
BAYAN_FAQ = [
    {
        "keywords": ["ما هي بيان", "ما هي منصة بيان", "عن المنصة", "ايش بيان", "تعريف"],
        "answer": (
            "منصة بَيَان هي نموذج أولي (Prototype) للرصد المبكر للمعلومات المضللة "
            "المتعلقة بالخدمات الحكومية، وتسريع مسار الرد الرسمي عليها — من الرصد "
            "إلى التحقق إلى تقييم الخطورة إلى القرار إلى الاعتماد البشري والنشر."
        ),
    },
    {
        "keywords": ["risk score", "درجة الخطورة", "الخطورة تحسب", "كيف تحسب الخطورة"],
        "answer": (
            "درجة الخطورة (Risk Score) تُحسب رياضيًا من 5 عوامل: سرعة الانتشار، "
            "حجم الجمهور، حساسية الخدمة، التعارض مع المصدر الرسمي، وقرب الموعد "
            "المذكور بالادعاء. التفاصيل الكاملة متاحة في صفحة «محرك المخاطر»."
        ),
    },
    {
        "keywords": ["من ينشر", "من يعتمد", "قرار النشر", "بشري", "human"],
        "answer": (
            "الذكاء الاصطناعي في بَيَان يقترح فقط (مسودة توضيح، توصية استجابة)، "
            "أما القرار والاعتماد والنشر النهائي فهو دائمًا بيد الموظف المختص "
            "(Human-in-the-Loop) — لا يوجد نشر تلقائي في أي مرحلة."
        ),
    },
    {
        "keywords": ["بلاغ", "استفسار", "كيف أقدم بلاغ", "تواصل", "شكوى"],
        "answer": (
            "يمكنك تقديم بلاغ أو استفسار مباشرة عبر لوحة «البلاغات والاستفسارات» "
            "في الجهة اليمنى من الشاشة — اختر نوع الطلب، واكتب التفاصيل وبيانات "
            "التواصل، ثم اضغط إرسال."
        ),
    },
    {
        "keywords": ["بيانات حقيقية", "demo", "تجريبي", "بيانات فعلية", "حقيقة"],
        "answer": (
            "هذا نموذج أولي لأغراض العرض فقط، وجميع البيانات المعروضة (الادعاءات، "
            "المصادر، الأحداث) بيانات محاكاة (Mock Data) ولا ترتبط بأي مصدر حكومي "
            "أو منصة تواصل اجتماعي حقيقية."
        ),
    },
    {
        "keywords": ["زمن الاستجابة", "كم يستغرق", "سرعة الاستجابة"],
        "answer": (
            "بَيَان يهدف لاختصار زمن الاستجابة من الرصد إلى النشر الرسمي. يمكنك "
            "الاطلاع على تفصيل الأزمنة الفعلية لكل حالة من صفحة «دورة الاستجابة»."
        ),
    },
    {
        "keywords": ["priority", "أولوية القرار", "p1", "p2", "p3", "p4"],
        "answer": (
            "أولوية القرار (Decision Priority) تصنّف كل حالة إلى أربع مستويات: "
            "P1 تدخل فوري، P2 تدخل عاجل، P3 مراقبة، P4 منخفضة الأولوية — بناءً على "
            "مزيج من الخطورة وسرعة الانتشار وحساسية الخدمة والثقة في التصنيف."
        ),
    },
    {
        "keywords": ["مساعدة", "help", "ماذا تستطيع", "ماذا يمكنك"],
        "answer": (
            "أقدر أجاوبك على أسئلة عامة حول منصة بَيَان: طريقة عمل المنصة، "
            "الخطورة، القرار، زمن الاستجابة، وكيفية تقديم بلاغ أو استفسار. "
            "لأي طلب أكثر تفصيلاً استخدم لوحة «البلاغات والاستفسارات»."
        ),
    },
]

FAQ_FALLBACK = (
    "لم أجد إجابة مباشرة لسؤالك ضمن قاعدة المعرفة الحالية. يمكنك تقديم استفسارك "
    "رسميًا عبر لوحة «البلاغات والاستفسارات» على يمين الشاشة وسيتم الرد عليك من "
    "الفريق المختص."
)


def match_faq(question: str) -> str:
    """مطابقة بسيطة بالكلمات المفتاحية داخل قاعدة الأسئلة الثابتة."""
    q = (question or "").strip().lower()
    if not q:
        return FAQ_FALLBACK
    for item in BAYAN_FAQ:
...
REPORT_TYPES = ["استفسار عام", "بلاغ عن معلومة مضللة", "بلاغ عن حساب منتحل", "ملاحظة على المنصة"]


def init_support_state():
    if "show_report_panel" not in st.session_state:
        st.session_state.show_report_panel = False
    if "show_chat_panel" not in st.session_state:
        st.session_state.show_chat_panel = False
    if "reports_log" not in st.session_state:
        st.session_state.reports_log = []
    if "bayan_chat_history" not in st.session_state:
        st.session_state.bayan_chat_history = [
            {"role": "bot", "text": "أهلًا بك 👋 أنا بَيَان، كيف أقدر أساعدك؟"}
        ]


def inject_support_css(colors: dict) -> str:
    """يرجع CSS التموضع الثابت للوحتين، لدمجه ضمن CSS الرئيسي للمنصة."""
    return f"""
    /* ===== زر وبطاقة البلاغات والاستفسارات - يمين الشاشة ===== */
    div.st-key-report_toggle_btn {{
        position: fixed;
        top: 110px;
        right: 0;
        z-index: 999998;
    }}
    div.st-key-report_toggle_btn button {{
    ...
    /* ===== زر ونافذة المساعد الآلي بَيَان - أسفل يسار الشاشة ===== */
    ...
    div.st-key-chat_toggle_btn button {{
        ...
        background-color: {colors['ai_purple']} !important;
        color: white !important;
        border: none !important;
    ...
    div.st-key-chat_panel {{
        ...
        color: {colors['text']};
        border: 1px solid {colors['border']};
        border-top: 4px solid {colors['ai_purple']};
        border-radius: 12px;
        padding: 0.9rem;
        z-index: 999999;
        box-shadow: 0 6px 20px rgba(0,0,0,0.2);
    }}
    .bayan-chat-bubble-bot {{
    ...
    .bayan-chat-bubble-user {{
        ...
        font-size: 0.85rem;
        text-align: right;
        color: {colors['ai_purple']};
        font-weight: 600;
    }}
    """


def render_report_panel(colors: dict):
    """لوحة البلاغات والاستفسارات - عائمة يمين الشاشة، تظهر في كل الصفحات."""
    with st.container(key="report_toggle_btn"):
        label = "✕ إغلاق" if st.session_state.show_report_panel else "📢 البلاغات والاستفسارات"
        if st.button(label, key="report_toggle_btn_inner"):
            st.session_state.show_report_panel = not st.session_state.show_report_panel
            st.rerun()

    if not st.session_state.show_report_panel:
        return

    with st.container(key="report_panel"):
        st.markdown(
            f"""<div style="font-weight:800; color:{colors['primary_dark']}; margin-bottom:0.4rem;">
            البلاغات والاستفسارات</div>""",
            unsafe_allow_html=True,
        )
        with st.form("report_form", clear_on_submit=True):
            r_type = st.selectbox("نوع الطلب", REPORT_TYPES)
            r_subject = st.text_input("الموضوع")
            r_details = st.text_area("التفاصيل", height=100)
            r_contact = st.text_input("بيانات التواصل (بريد إلكتروني أو جوال)")
            submitted = st.form_submit_button("إرسال")

        if submitted:
            if not r_subject or not r_details:
                st.warning("الرجاء تعبئة الموضوع والتفاصيل قبل الإرسال.")
            else:
                st.session_state.reports_log.append({
                    "type": r_type,
                    "subject": r_subject,
                    "details": r_details,
                    "contact": r_contact,
                ...
                st.success("تم استلام طلبك بنجاح، سيتم التواصل معك من الفريق المختص.")

        if st.session_state.reports_log:
            st.caption(f"عدد الطلبات المرسلة في هذه الجلسة: {len(st.session_state.reports_log)}")


def render_chat_widget(colors: dict):
    """المساعد الآلي بَيَان - عائم أسفل يسار الشاشة، يظهر في كل الصفحات."""
    with st.container(key="chat_toggle_btn"):
        label = "✕ إغلاق" if st.session_state.show_chat_panel else "💬 بَيَان"
        if st.button(label, key="chat_toggle_btn_inner"):
    ...
    with st.container(key="chat_panel"):
        st.markdown(
            f"""<div style="font-weight:800; color:{colors['ai_purple']}; margin-bottom:0.4rem;">
            💬 المساعد الآلي — بَيَان</div>""",
            unsafe_allow_html=True,
        )

        for msg in st.session_state.bayan_chat_history[-12:]:
            css_class = "bayan-chat-bubble-bot" if msg["role"] == "bot" else "bayan-chat-bubble-user"
            st.markdown(f'<div class="{css_class}">{msg["text"]}</div>', unsafe_allow_html=True)

        with st.form("chat_form", clear_on_submit=True):
            user_q = st.text_input("اكتب سؤالك هنا...", label_visibility="collapsed",
                                    placeholder="اكتب سؤالك هنا...")
            sent = st.form_submit_button("إرسال")

        if sent and user_q.strip():
            st.session_state.bayan_chat_history.append({"role": "user", "text": user_q.strip()})
            answer = match_faq(user_q)
            st.session_state.bayan_chat_history.append({"role": "bot", "text": answer})
            st.rerun()


def render_support_widgets(colors: dict):
    """نقطة الدخول الوحيدة: تُستدعى من app.py بعد عرض أي صفحة."""
    init_support_state()
    render_report_panel(colors)
    render_chat_widget(colors)
...
# ألوان الهوية البصرية للمنصة: بنفسجي أساسي + تركوازي مساعد لعناصر الذكاء
# الاصطناعي، مع دعم وضعين (ليلي/نهاري). دوال الصفحات تقرأ القيم من القاموس
# العام COLORS الذي تتم تعبئته عبر apply_theme() في بداية كل تشغيل للسكربت.
THEMES = {
    "light": {
        "primary": "#6D3FC2",       # بنفسجي أساسي للهوية
        "primary_dark": "#4E2A94",  # بنفسجي غامق للتباين والعناوين
        "secondary": "#1F2233",     # لون الشريط الجانبي الداكن (ثابت في الوضعين)
        "bg": "#FFFFFF",
        "bg_light": "#F7F5FC",      # خلفية عامة بلمسة بنفسجية فاتحة جدًا
        "surface": "#FFFFFF",       # خلفية البطاقات واللوحات
        "surface_alt": "#F1EAFB",   # خلفية فاتحة لعناصر الذكاء الاصطناعي (بنرات/فقاعات)
        "text": "#1F2233",
        "accent": "#14B8A6",        # تركوازي - اللون المساعد للهوية
        "ai_purple": "#0D9488",     # تركوازي غامق لعناصر «بَيَان الذكي» تمييزًا عن البنفسجي الأساسي
        "success": "#2E7D32",
        "warning": "#B8860B",
        "high": "#C1440E",
        "critical": "#B00020",
    ...
    "dark": {
        "primary": "#9B6FE0",       # بنفسجي أفتح لضمان تباين جيد على خلفية داكنة
        "primary_dark": "#C9AFF2",
        "secondary": "#14162A",     # الشريط الجانبي أدكن قليلًا في الوضع الليلي
        "bg": "#12131F",
        "bg_light": "#181A2C",
        "surface": "#1F2137",       # خلفية البطاقات في الوضع الليلي
        "surface_alt": "#2A2650",   # خلفية عناصر الذكاء الاصطناعي في الوضع الليلي
        "text": "#EDEDF7",
        "accent": "#2DD4BF",        # تركوازي أفتح ليبرز على الخلفية الداكنة
        "ai_purple": "#2DD4BF",
        "success": "#4CAF50",
        "warning": "#D9A441",
        "high": "#E2703A",
...
COLORS = dict(THEMES["light"])  # تُحدَّث فعليًا داخل apply_theme() بحسب اختيار المستخدم


def apply_theme():
    """يحدّث قاموس COLORS العام بحسب الوضع الحالي (ليلي/نهاري) المخزّن في
    session_state. يُستدعى في بداية كل تشغيل للسكربت قبل حقن CSS، وبما أن
    COLORS نفس الكائن (dict) الذي تستخدمه كل الدوال الأخرى في الملف، فإن أي
    تعديل هنا ينعكس تلقائيًا على كامل الواجهة والرسوم البيانية."""
    theme_name = st.session_state.get("theme", "light")
    COLORS.clear()
    COLORS.update(THEMES.get(theme_name, THEMES["light"]))
...
def inject_css():
    st.markdown(
        ...
        .pill-ai {{ background:{COLORS['surface_alt']}; color:{COLORS['ai_purple']}; }}
        .pill-neutral {{ background:{COLORS['border']}; color:{COLORS['text']}; }}
        .bayan-banner {{
            background: {COLORS['surface_alt']};
...
def render_risk_breakdown_chart(breakdown):
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(y=labels, x=maxpts, orientation="h", marker_color=COLORS["border"], name="الحد الأقصى", showlegend=False))
    ...
    fig.update_layout(
        ...
        xaxis_title="النقاط", plot_bgcolor=COLORS["surface"], paper_bgcolor=COLORS["surface"],
        font=dict(family="Tahoma", color=COLORS["text"]),
    )
    return fig
...
# =============================================================================
def journey_step_header(number, title, subtitle="", connect_before=True):
    """يعرض رأس خطوة داخل «رحلة الحالة» بشكل دائرة مرقّمة موصولة بخط، لبناء
    سرد بصري متسلسل من الرصد إلى التوثيق ضمن صفحة واحدة."""
    if connect_before:
        st.markdown(
            f'<div style="width:2px; height:16px; background:{COLORS["border"]}; margin-right:19px;"></div>',
            unsafe_allow_html=True,
        )
    st.markdown(
        f"""
        <div style="display:flex; align-items:center; gap:0.8rem; margin-bottom:0.6rem;">
            <div style="min-width:38px; height:38px; border-radius:50%;
                        background:linear-gradient(135deg, {COLORS['primary']}, {COLORS['ai_purple']});
                        color:white; display:flex; align-items:center; justify-content:center;
                        font-weight:800; font-size:1rem; box-shadow:0 2px 6px rgba(0,0,0,0.15);">
                {number}
            </div>
            <div>
                <div style="font-weight:800; font-size:1.15rem; color:{COLORS['text']};">{title}</div>
                {f'<div style="font-size:0.8rem; color:{COLORS["muted"]};">{subtitle}</div>' if subtitle else ''}
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
def sidebar_nav():
    st.sidebar.markdown(
        ...
        <div style="text-align:center; padding:0.6rem 0 0.3rem 0;">
            ...
            <div style="font-size:0.8rem; color:{COLORS['accent']}; letter-spacing:2px;">BAYAN</div>
    ...
    theme_icon = "☀️ الوضع النهاري" if st.session_state.theme == "dark" else "🌙 الوضع الليلي"
    if st.sidebar.button(theme_icon, key="theme_toggle_btn"):
        st.session_state.theme = "light" if st.session_state.theme == "dark" else "dark"
        st.rerun()
    st.sidebar.markdown("<div style='margin-bottom:0.4rem;'></div>", unsafe_allow_html=True)

    def nav_button(label):
        active = st.session_state.page == label
        prefix = "▸ " if active else "‎ ‎ "
        if st.sidebar.button(f"{prefix}{label}", key=f"nav_{label}"):
            st.session_state.page = label
            st.rerun()

    nav_button("الرئيسية")
    nav_button("رحلة الحالة")
...
# =============================================================================
# =============================================================================
# صفحة: رحلة الحالة الكاملة (Case Journey) — سرد بصري متسلسل واحد
# من الرصد إلى التوثيق، بدل توزيع دورة القرار على صفحات منفصلة.
# =============================================================================
def page_case_journey():
    df = get_claims_df()
    st.title("رحلة الحالة الكاملة")
    st.caption("رصد ← تحليل ← تحقق ← تصنيف ← إجراء ← رد ← اعتماد ← توثيق — في مسار واحد لكل حالة.")

    claim_id = st.selectbox(
        "اختر حالة لعرض رحلتها الكاملة",
        df["id"].tolist(),
        index=df["id"].tolist().index(st.session_state.selected_claim) if st.session_state.selected_claim in df["id"].tolist() else 0,
        format_func=lambda x: f"{x} — {df[df['id']==x]['content'].values[0][:60]}",
        key="journey_claim_select",
    )
    st.session_state.selected_claim = claim_id
    ...
    st.markdown(
        f"""<div class="bayan-card">
            <div class="bayan-card-title">{row['content']}</div>
            <div>{risk_pill(row['risk_score'])} {priority_pill(row['priority_code'], row['priority_label'])} {status_pill(row['response_status'])}</div>
        </div>""",
        unsafe_allow_html=True,
    )

    # 1) الرصد -----------------------------------------------------------
    journey_step_header("1", "الرصد", "اكتشاف المحتوى المتداول من مصادره", connect_before=False)
    c1, c2, c3 = st.columns(3)
    c1.metric("المصدر", row["source"])
    c2.metric("حجم الوصول الحالي", f"{int(row['reach']):,}")
    c3.metric("معدل الانتشار", f"{row['growth_rate']}%/ساعة")
    st.caption(f"وقت الرصد: {row['timestamp']}")

    # 2) التحليل ----------------------------------------------------------
    journey_step_header("2", "التحليل", "استخراج عناصر الادعاء وتوقع مسار الانتشار")
    ext = extract_claim(row)
    ec1, ec2 = st.columns([1.3, 1])
    with ec1:
        st.markdown(
            f"- **الجهة المرتبطة:** {ext['agency']}\n"
            f"- **الخدمة المعنية:** {ext['service']}\n"
            f"- **نوع الادعاء الأولي:** {ext['claim_type']}\n"
            f"- **الموعد المذكور بالادعاء:** {ext['mentioned_date']}"
        )
    with ec2:
        preds = predict_spread(row["reach"], row["growth_rate"])
        st.metric("الوصول المتوقع خلال 12 ساعة (دون تدخل)", f"{preds[12]:,}")

    # 3) التحقق -----------------------------------------------------------
    journey_step_header("3", "التحقق", "مطابقة الادعاء مع المصادر الرسمية")
    v = verify_claim(row)
    st.markdown(status_pill(v["status"]), unsafe_allow_html=True)
    for r in v["reasons"]:
        st.markdown(f"- {r}")
    if evidence_list:
        with st.expander(f"الأدلة المرتبطة ({len(evidence_list)})"):
            for e in evidence_list:
                st.markdown(f"- **{e['source_name']}** ({e['source_type']}) — {e['description']}")
    else:
        st.caption("لا توجد أدلة موثّقة مرتبطة بهذه الحالة بعد.")

    # 4) التصنيف والخطورة --------------------------------------------------
    journey_step_header("4", "التصنيف وتقييم الخطورة", "درجة خطورة قابلة للتفسير + أولوية القرار")
    breakdown = compute_risk_breakdown(row)
    priority = compute_decision_priority(row)
    rc1, rc2 = st.columns([1.4, 1])
    with rc1:
        st.plotly_chart(render_risk_breakdown_chart(breakdown), use_container_width=True, key=f"journey_risk_{claim_id}")
    with rc2:
        st.metric("Risk Score", f"{breakdown['total']}/100")
        st.markdown(priority_pill(priority["priority_code"], priority["priority_label"]), unsafe_allow_html=True)

    # 5) الإجراء الموصى به --------------------------------------------------
    journey_step_header("5", "الإجراء الموصى به", "توصية قائمة على قواعد واضحة (Rule-Based)")
    rec = recommend_action(row)
    st.markdown(f"**التوصية:** {rec['option']['title']} — قناة مقترحة حسب نوع الادعاء")
    for r in rec["reasons"]:
        st.markdown(f"- {r}")
    st.caption(f"ما الذي قد يغيّر هذه التوصية؟ {rec['what_could_change']}")

    # 6) الرد المقترح -------------------------------------------------------
    journey_step_header("6", "الرد المقترح", "مسودة توضيح يولّدها بَيَان — تبقى مقترحة دومًا")
    st.markdown('<div class="bayan-banner">الذكاء الاصطناعي يقترح — والقرار النهائي للجهة المختصة.</div>', unsafe_allow_html=True)
    if st.button("توليد / تحديث مسودة الرد", key=f"journey_gen_{claim_id}"):
        st.session_state.drafts[claim_id] = generate_response(row, row["agency_name"], evidence_list)
        st.rerun()

    draft = st.session_state.drafts.get(claim_id)
    if draft:
        edited_title = st.text_input("عنوان المسودة", draft["title"], key=f"journey_title_{claim_id}")
        edited_statement = st.text_area("نص التوضيح الرسمي", draft["statement"], height=120, key=f"journey_stmt_{claim_id}")
        if st.button("حفظ التعديل على المسودة", key=f"journey_save_{claim_id}"):
        ...
        with st.expander("الحقائق الأساسية والقناة المقترحة"):
            for kf in draft["key_facts"]:
                st.markdown(f"- {kf}")
            st.markdown(f"**القناة المقترحة للنشر:** {draft['recommended_channel']}")
    else:
        st.caption("لم يتم توليد مسودة بعد لهذه الحالة.")

    # 7) الاعتماد البشري -----------------------------------------------------
    journey_step_header("7", "الاعتماد البشري", "لا نشر رسمي إلا بعد اعتماد صريح من الموظف المختص")
    approval = st.session_state.approvals.get(claim_id, {"status": row["response_status"]})
    st.markdown(f"**الحالة الحالية:** {status_pill(approval.get('status', row['response_status']))}", unsafe_allow_html=True)
    a1, a2, a3 = st.columns(3)
    if a1.button("اعتماد", key=f"journey_approve_{claim_id}", disabled=draft is None):
        ...
        st.rerun()
    if a2.button("رفض", key=f"journey_reject_{claim_id}", disabled=draft is None):
        st.session_state.approvals[claim_id] = {
            "status": "مرفوض", "reviewer": REVIEWER_NAME,
            "approval_time": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "decision": "رفض",
        }
        st.session_state.claims_runtime[claim_id]["response_status"] = "مرفوض"
        st.rerun()
    if a3.button("طلب تعديل", key=f"journey_revise_{claim_id}", disabled=draft is None):
    ...
    if approval.get("status") == "معتمد ومنشور" and "reviewer" in approval:
        st.success(f"معتمد بشريًا بواسطة {approval['reviewer']} ({REVIEWER_ROLE}) بتاريخ {approval['approval_time']}")

    # 8) التوثيق والإغلاق -----------------------------------------------------
    journey_step_header("8", "التوثيق", "سجل تدقيق كامل قابل للمراجعة من الرصد إلى الإغلاق")
    rt = render_timeline(claim_id)
    if rt and rt.get("total") is not None:
        st.metric("إجمالي زمن الاستجابة المسجّل", f"{rt['total']} دقيقة")
    st.markdown(
        '<div class="footer-note">جميع الأزمنة والبيانات في هذا العرض تجريبية (Mock Data) لأغراض الهاكاثون، '
        'ولا تعكس اتصالًا فعليًا بأي مصدر حكومي حقيقي.</div>',
        unsafe_allow_html=True,
    )
...
def page_home():
    ...
    with col1:
        if st.button("🧭 رحلة الحالة الكاملة", use_container_width=True):
            go_to("رحلة الحالة")
    with col2:
    ...
    st.markdown(
        f"""
        <div style="text-align:center; margin:1.2rem 0 2rem 0; color:{COLORS['text']}; font-size:1.05rem; font-weight:600;">
...
def page_dashboard():
    ...
    with left:
        ...
        fig.update_layout(height=340, plot_bgcolor=COLORS["surface"], paper_bgcolor=COLORS["surface"],
                           xaxis_title="الوقت", yaxis_title="حجم الوصول (Reach)", font=dict(family="Tahoma", color=COLORS["text"]))
    ...
    with right:
        ...
        fig2.update_layout(height=340, plot_bgcolor=COLORS["surface"], paper_bgcolor=COLORS["surface"],
                            yaxis_title="", xaxis_title="عدد الحالات", font=dict(family="Tahoma", color=COLORS["text"]))
...
def page_investigation():
    ...
    # 3) التحقق الذكي
    st.markdown("### 3. هل هو مضلل؟ — التحقق الذكي")
    v = verify_claim(row)
    ...
    # 4) الأدلة
    st.markdown("### 4. الأدلة — Evidence Engine")
    ...
    # 5) Risk Engine
    st.markdown("### 5. Risk — درجة الخطورة")
    breakdown = compute_risk_breakdown(row)
    ...
    # 6) التنبؤ بالانتشار
    st.markdown("### 6. ماذا لو لم نتدخل؟ — التنبؤ بالانتشار")
    preds = predict_spread(row["reach"], row["growth_rate"])
    ...
    st.markdown(f'<div class="bayan-banner">نافذة التدخل المثلى: {window}</div>', unsafe_allow_html=True)
    ...
    fig.update_layout(height=300, xaxis_title="الساعات", yaxis_title="الوصول المتوقع", plot_bgcolor=COLORS["surface"],
                       paper_bgcolor=COLORS["surface"], font=dict(family="Tahoma", color=COLORS["text"]), legend=dict(orientation="h", y=-0.2))
    st.plotly_chart(fig, use_container_width=True)
    st.caption("نموذج رياضي مبسّط مبني على بيانات تجريبية — ليس نموذج تنبؤ إنتاجي حقيقي.")

    # 7) محرك القرار
    st.markdown("### 7. ماذا يجب أن تفعل الجهة الآن؟")
    rec = recommend_action(row)
    ...
    # 8) مولّد المسودة
    st.markdown("### 8. مولّد المسودة الرسمية")
    if st.button("إنشاء مسودة توضيح رسمي", key=f"gen_{claim_id}"):
    ...
    if claim_id in st.session_state.drafts:
        ...
        if st.button("الانتقال لمراجعة المسودة", key=f"goto_review_{claim_id}"):
...
def page_risk_engine():
    ...
    fig.update_layout(height=300, plot_bgcolor=COLORS["surface"], paper_bgcolor=COLORS["surface"], showlegend=False, font=dict(family="Tahoma", color=COLORS["text"]))
    st.plotly_chart(fig, use_container_width=True)
...
def page_decision_engine():
    ...
    fig.update_layout(height=280, plot_bgcolor=COLORS["surface"], paper_bgcolor=COLORS["surface"], showlegend=False, font=dict(family="Tahoma", color=COLORS["text"]))
    st.plotly_chart(fig, use_container_width=True)
...
def page_responses():
    ...
    if b5.button("طلب تعديل", key=f"revise_{claim_id}"):
        st.session_state.approvals[claim_id] = {"status": "قيد المراجعة البشرية"}
        st.session_state.claims_runtime[claim_id]["response_status"] = "قيد المراجعة البشرية"
        st.info("تم تحويل المسودة لطلب تعديل إضافي.")
...
def page_response_cycle():
    ...
    fig.update_layout(height=300, plot_bgcolor=COLORS["surface"], paper_bgcolor=COLORS["surface"], yaxis_title="الدقائق", font=dict(family="Tahoma", color=COLORS["text"]))
    st.plotly_chart(fig, use_container_width=True)
...
def page_impact():
    ...
    fig.update_layout(height=340, xaxis_title="الساعات", yaxis_title="الوصول (Reach)",
                       plot_bgcolor=COLORS["surface"], paper_bgcolor=COLORS["surface"], font=dict(family="Tahoma", color=COLORS["text"]))
    st.plotly_chart(fig, use_container_width=True)
...
def page_demo():
    ...
    with nav1:
        if step > 0 and st.button("السابق"):
            st.session_state.demo_step -= 1
            st.rerun()
    with nav2:
        if step < len(DEMO_STEPS) - 1 and st.button("التالي"):
            st.session_state.demo_step += 1
            st.rerun()
...
# لوحة البلاغات والاستفسارات (يمين الشاشة) + المساعد الآلي بَيَان (أسفل يسار الشاشة)
# تُستدعى بعد كل صفحة حتى تبقى ثابتة ومتاحة في جميع صفحات المنصة
render_support_widgets(COLORS)
